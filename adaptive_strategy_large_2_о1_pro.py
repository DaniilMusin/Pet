...
class BalancedAdaptiveStrategy:
    def __init__(self, data_path, 
                 initial_balance=1000, max_leverage=2, 
        ...
        self.params = {
            ...
            'pyramid_min_profit': 0.01,  # Пирамидинг чаще
            'pyramid_size_multiplier': 0.7,  # Больше добавляем к позиции
            'max_pyramid_entries': 5,  # Больше пирамидинга
        ...
        self.max_daily_loss_pct = 0.05  # Дневной лимит убытков: -5%
        self.max_weekly_loss_pct = 0.15  # Недельный лимит: -15%
        self.daily_pnl = {}  # Словарь для отслеживания дневных P&L
        self.weekly_pnl = {}  # Словарь для отслеживания недельных P&L
    ...
    def load_data(self):
        ...
        # Sample more data to increase trade opportunities
        self.data = self.data.tail(30000)  # Increase from 20000
    ...
    def adapt_to_market_conditions(self):
        ...
        short_signal_threshold = 0.6  # Lower threshold for shorts
        ...
        self.data['Balanced_Short_Signal'] = (
            (self.data['Final_Short_Bias'] > short_signal_threshold) |  # Lower threshold
    ...
    def adaptive_risk_per_trade(self, current_market_regime, win_rate_long, win_rate_short):
        ...
        # Адаптивный риск на сделку в зависимости от рыночного режима
        regime_factors = {
            "strong_bull": {"LONG": 1.5, "SHORT": 1.0},
            "strong_bear": {"LONG": 1.0, "SHORT": 1.5},
            "choppy_range": {"LONG": 1.2, "SHORT": 1.2},
            "transition_to_bull": {"LONG": 1.3, "SHORT": 1.2},
            "transition_to_bear": {"LONG": 1.2, "SHORT": 1.4},
            "mixed": {"LONG": 2.0, "SHORT": 2.0},
        ...
        return {
            "LONG": base_risk * regime_factors[current_market_regime]["LONG"],
            "SHORT": base_risk * regime_factors[current_market_regime]["SHORT"]
        }
    ...
    def apply_advanced_filtering(self, current, signals):
        ...
        # Менее строгие ограничения для шортов
        if 'Market_Health' in current:
            # Ограничиваем шорты при очень высоком здоровье рынка
            if current['Market_Health'] > 70:  # Было 40, увеличиваем порог
                short_weight *= 0.3  # Ослабляем, но не убираем полностью
            
        # Шорты предпочтительнее при нисходящем тренде
        if 'MTF_Trend' in current:
            if current['MTF_Trend'] < -0.2:  # Нисходящий тренд
                short_weight *= 1.5
            elif current['MTF_Trend'] > 0.2:  # Восходящий тренд
                short_weight *= 0.5
        
        # Шорты в определенных режимах рынка
        if 'Market_Regime' in current:
            regime = current['Market_Regime']
            if regime in ['strong_bear', 'transition_to_bear']:
                short_weight *= 1.5
                long_weight *= 0.7
            elif regime in ['strong_bull', 'transition_to_bull']:
                long_weight *= 1.5
                short_weight *= 0.7
            elif regime == 'mixed':
                long_weight *= 1.2
        ...
        if len(signals['short_signals']) == 0:
            short_weight = 0
        if len(signals['long_signals']) == 0:
            long_weight = 0
    ...
    def calculate_dynamic_exit_levels(self, position_type, entry_price, current_candle, trade_age_hours=0):
        # Получаем ATR для динамических стоп-лоссов и тейк-профитов
        atr = current_candle.get('ATR', entry_price * 0.02)  # По умолчанию 2%, если ATR недоступен
        
        # Увеличенные множители для более безопасных стопов
        sl_multiplier = self.params['atr_multiplier_sl'] * 1.5  # Увеличиваем множитель до 3.75 (2.5 * 1.5)
        tp_multiplier = self.params['atr_multiplier_tp'] * 0.9  # Уменьшаем тейк-профит для быстрого закрытия прибыльных сделок
        
        # Корректировка на основе возраста сделки - ужесточаем стопы для старых сделок
        if trade_age_hours > 12:
            sl_multiplier *= 0.8  # Ужесточаем стопы на 20% для сделок старше 12 часов
            
        # Разные уровни выхода для лонгов и шортов
        if position_type == 'LONG':
            stop_loss = entry_price - (atr * sl_multiplier)
            take_profit = entry_price + (atr * tp_multiplier)
        else:  # SHORT
            # Защитные параметры для шортов
            stop_loss = entry_price + (atr * sl_multiplier * 1.2)  # Еще шире стопы для шортов
            take_profit = entry_price - (atr * tp_multiplier * 0.8)  # Ближе тейк-профит
        
        # Проверка на минимальное соотношение риск/прибыль
        if position_type == 'LONG':
        ...
        # Если R:R меньше 1.5, корректируем тейк-профит
    ...
    def apply_trailing_stop(self, position_type, entry_price, current_price, max_price, min_price, unrealized_pnl_pct):
        ...
        # Store current market regime for position sizing
        if hasattr(self, 'data') and 'Market_Regime' in self.data.columns:
            current_index = self.data.index[self.data['Close'] == current_price].tolist()
            if current_index:
                self.current_market_regime = self.data.loc[current_index[0], 'Market_Regime']
        
        # More aggressive trailing stops to lock in profits
        if position_type == 'LONG':
            if unrealized_pnl_pct >= 0.03:
                if unrealized_pnl_pct >= 0.10:
                    trail_pct = unrealized_pnl_pct * 0.6  # Lock in more profit (was 0.4)
                    new_stop = entry_price * (1 + trail_pct * 0.8)  # Move stop closer to current price
                elif unrealized_pnl_pct >= 0.05:
                    trail_pct = unrealized_pnl_pct * 0.5  # Was 0.3
                    new_stop = entry_price * (1 + trail_pct * 0.7)
                else:
                    trail_pct = unrealized_pnl_pct * 0.4  # Was 0.2
                    new_stop = entry_price * (1 + trail_pct * 0.6)
                return new_stop
    ...
    def calculate_optimal_leverage(self, current_candle, trade_direction, max_allowed_leverage=2):
        # Снижаем плечо для всех сделок
        return 1.5  # Еще более консервативное значение, чем максимально разрешенное
        
    def adaptive_position_sizing(self, balance, risk_per_trade, entry_price, stop_loss_price, optimal_leverage):
        # Базовые расчеты размера позиции
        price_risk_pct = abs(entry_price - stop_loss_price) / entry_price
        
        # Расчет суммы риска с учетом лимитов
        risk_amount = balance * risk_per_trade
        
        # Ограничение риска на основе дневных/недельных лимитов убытков
        ...
        if day_key in self.daily_pnl and self.daily_pnl[day_key] < 0:
            # Если уже есть убытки за день, снижаем риск
            day_loss_ratio = abs(self.daily_pnl[day_key]) / (balance * self.max_daily_loss_pct)
            if day_loss_ratio > 0.5:  # Если использовано более 50% дневного лимита
                risk_amount *= max(0.1, 1 - day_loss_ratio)  # Снижаем риск пропорционально использованному лимиту
        
        # Проверка недельного лимита
        week_key = f"{current_date.year}-W{current_week}"
        if week_key in self.weekly_pnl and self.weekly_pnl[week_key] < 0:
            # Если уже есть убытки за неделю, снижаем риск
            week_loss_ratio = abs(self.weekly_pnl[week_key]) / (balance * self.max_weekly_loss_pct)
            if week_loss_ratio > 0.5:  # Если использовано более 50% недельного лимита
                risk_amount *= max(0.1, 1 - week_loss_ratio)  # Снижаем риск пропорционально использованному лимиту
        
        # Снижаем риск при просадках
        if hasattr(self, 'max_balance') and self.max_balance > 0:
            drawdown = 1 - (balance / self.max_balance)
            if drawdown > 0.05:  # Если в >5% просадке (снижено с 10%)
                risk_reduction = min(0.5, 1 - (drawdown * 3))  # Более агрессивное снижение риска
                risk_amount *= risk_reduction
        
        # Расчет размера позиции
        if price_risk_pct > 0:
            position_size_no_leverage = risk_amount / price_risk_pct
        else:
            position_size_no_leverage = 0
        
        # Применение плеча
        position_size = position_size_no_leverage * optimal_leverage
        
        # Ограничение максимального размера позиции как процента от баланса
        max_position = balance * optimal_leverage * 0.5  # Снижено с 0.95 до 0.5
        position_size = min(position_size, max_position)
        
        return position_size
    
    def is_optimal_trading_time(self, timestamp):
        # Всегда разрешаем сделки
        return True
    ...
    def analyze_hour_performance(self):
        ...
        """
        if not hasattr(self, 'trade_df') or self.trade_df is None or len(self.trade_df) == 0:
            print("Trade DataFrame не инициализирован или недостаточно данных.")
            return None
    ...
    def run_backtest(self):
        ...
        self.max_balance = self.initial_balance  # Track maximum balance for drawdown calculations
        position = 0
        entry_price = 0.0
        position_size = 0.0
        position_type = None
        entry_time = None
        trade_signals = []
        trade_exit_levels = None
        max_price = 0.0
        min_price = float('inf')
        last_trade_index = -100  # For enforcing trade interval
        ...
        for i in range(1, len(self.data)):
            ...
            # Проверка дневных и недельных лимитов убытков
            ...
            # Проверяем, не превышены ли лимиты убытков
            if self.daily_pnl[day_key] <= -balance * self.max_daily_loss_pct:
                # Дневной лимит убытков превышен, пропускаем день
                        continue
                    
            if self.weekly_pnl[week_key] <= -balance * self.max_weekly_loss_pct:
                # Недельный лимит убытков превышен, пропускаем день
                        continue
                    
            # Если у нас открыта позиция
            closed_pnl = 0.0
            if position != 0:
                # Отслеживаем максимальную и минимальную цену для трейлинг-стопов
                if current['High'] > max_price:
                    max_price = current['High']
                if current['Low'] < min_price:
                    min_price = current['Low']
                
                # Рассчитываем возраст сделки в часах
                if entry_time is not None:
                    trade_age_hours = (current.name - entry_time).total_seconds() / 3600
                    else:
                    trade_age_hours = 0
                
                # Обновляем уровни выхода на основе текущих условий рынка
                trade_exit_levels = self.calculate_dynamic_exit_levels(
                    position_type, entry_price, current, trade_age_hours
                )
                
                # Определяем нереализованную прибыль/убыток в процентах
                if position == 1:  # LONG
                    unrealized_pnl_pct = (current['Close'] / entry_price) - 1
                else:  # SHORT
                    unrealized_pnl_pct = 1 - (current['Close'] / entry_price)
                
                # Проверяем условия для трейлинг-стопа
                trailing_stop_price = self.apply_trailing_stop(
                    position_type, entry_price, current['Close'], max_price, min_price, unrealized_pnl_pct
                )
                
                # Определяем, нужно ли закрыть позицию
                close_position = False
                exit_reason = None
                exit_price = current['Close']
                
                # Проверка на стоп-лосс
                if position == 1 and current['Low'] <= trade_exit_levels['stop_loss']:
                    close_position = True
                    exit_reason = "Stop Loss"
                    exit_price = trade_exit_levels['stop_loss']
                elif position == -1 and current['High'] >= trade_exit_levels['stop_loss']:
                    close_position = True
                    exit_reason = "Stop Loss"
                    exit_price = trade_exit_levels['stop_loss']
                
                # Проверка на тейк-профит
                if position == 1 and current['High'] >= trade_exit_levels['take_profit']:
                    close_position = True
                    exit_reason = "Take Profit"
                    exit_price = trade_exit_levels['take_profit']
                elif position == -1 and current['Low'] <= trade_exit_levels['take_profit']:
                    close_position = True
                    exit_reason = "Take Profit"
                    exit_price = trade_exit_levels['take_profit']
                ...
                if trailing_stop_price is not None:
                    if position == 1 and current['Low'] <= trailing_stop_price:
                        close_position = True
                        exit_reason = "Trailing Stop"
                        exit_price = trailing_stop_price
                    elif position == -1 and current['High'] >= trailing_stop_price:
                        close_position = True
                        exit_reason = "Trailing Stop"
                        exit_price = trailing_stop_price
                
                # Закрываем позицию, если есть сигнал на закрытие
                if close_position:
                    # Рассчитываем P&L
                    if position == 1:  # LONG
                        pnl = ((exit_price / entry_price) - 1) * position_size
                    else:  # SHORT
                        pnl = (1 - (exit_price / entry_price)) * position_size
                    ...
                    trade_result = {
                        ...
                        'market_health': current.get('Market_Health', 50),
                    ...
                    # Обновляем дневные и недельные P&L
                    self.daily_pnl[day_key] += pnl
                    self.weekly_pnl[week_key] += pnl
            
            # Ищем новые сигналы для входа в позицию, если:
            # 1. У нас нет открытой позиции
            # 2. Мы соблюдаем минимальный интервал между сделками
            if position == 0 and (i - last_trade_index) >= self.min_trades_interval:
                # Получаем сигналы для текущей свечи
                signals = self.get_trading_signals(current, previous, current.get('Market_Regime', 'unknown'))
                
                # Применяем дополнительную фильтрацию
                filtered_signals = self.apply_advanced_filtering(current, signals)
                
                # Проверяем оптимальность времени для торговли
                optimal_time = self.is_optimal_trading_time(current.name)
                
                # Определяем, входить ли в лонг или шорт
                long_weight = filtered_signals['long_weight']
                short_weight = filtered_signals['short_weight']
                
                # Выбираем направление с наибольшим весом
                if optimal_time:
                    if long_weight > 0.5 and long_weight > short_weight:
                    ...
                    entry_price = current['Close']
                        ...
                        # Определяем оптимальное плечо
                        optimal_leverage = self.calculate_optimal_leverage(current, "LONG")
                        
                        # Адаптивный риск на сделку
                        risk_per_trade = self.adaptive_risk_per_trade(
                            current.get('Market_Regime', 'unknown'),
                            self.win_rate_long if hasattr(self, 'win_rate_long') else 0.5,
                            self.win_rate_short if hasattr(self, 'win_rate_short') else 0.5
                        )["LONG"]
                        
                        # Рассчитываем стоп-лосс
                        exit_levels = self.calculate_dynamic_exit_levels(position_type, entry_price, current)
                        stop_loss_price = exit_levels['stop_loss']
                        
                        # Определяем размер позиции
                        position_size = self.adaptive_position_sizing(
                            balance, risk_per_trade, entry_price, stop_loss_price, optimal_leverage
                        )
                        
                        # Запоминаем индекс сделки
                        last_trade_index = i
                        
                        # Сохраняем сигналы для анализа
                        trade_signals = signals['long_signals']
                        trade_exit_levels = exit_levels
                        
                        # Инициализируем max/min цены для трейлинг-стопа
                        max_price = current['High']
                        min_price = current['Low']
                        
                    elif short_weight > 0.5 and short_weight > long_weight:
                    ...
                    entry_price = current['Close']
                        ...
                        # Определяем оптимальное плечо
                        optimal_leverage = self.calculate_optimal_leverage(current, "SHORT")
                    ...
                    stop_loss_price = exit_levels['stop_loss']
                        ...
                        # Сохраняем сигналы для анализа
                        trade_signals = signals['short_signals']
                        trade_exit_levels = exit_levels
            ...
            # Обновляем статистику каждые 20 сделок
            if len(self.trade_history) % 20 == 0 and len(self.trade_history) > 0:
                self.dynamically_adjust_risk_parameters()
                
            # Сохраняем текущее состояние баланса и позиции
            equity = balance
            if position != 0 and entry_price > 0:
                if position == 1:  # LONG
                    unrealized_pnl = position_size * ((current['Close'] / entry_price) - 1)
                else:  # SHORT
                    unrealized_pnl = position_size * (1 - (current['Close'] / entry_price))
                equity = balance + unrealized_pnl
            
            # Сохраняем состояние в backtest_results
            self.backtest_results.append({
                'date': current.name,
                'balance': balance,
                'equity': equity,
                'position': position,
                'market_regime': current.get('Market_Regime', None),
                'market_health': current.get('Market_Health', None),
            })
            
        # Закрываем открытую позицию в конце бэктеста
        if position != 0 and entry_price > 0:
            ...
            trade_result = {
                ...
                'trade_duration': (self.data.index[-1] - entry_time).total_seconds() / 3600 if entry_time else 0,
                'market_regime': self.data.iloc[-1].get('Market_Regime', 'unknown'),
                'market_health': self.data.iloc[-1].get('Market_Health', 50),
                'adx': self.data.iloc[-1].get('ADX', 0),
                'rsi': self.data.iloc[-1].get('RSI', 50),
                'volume_ratio': self.data.iloc[-1].get('Volume_Ratio', 1.0),
                'atr': self.data.iloc[-1].get('ATR', 0),
            }
            self.trade_history.append(trade_result)
        
        # Преобразуем результаты в DataFrame
        if self.backtest_results:
            self.backtest_results = pd.DataFrame(self.backtest_results)
        else:
            self.backtest_results = pd.DataFrame(columns=['date', 'balance', 'equity', 'position', 'market_regime', 'market_health'])
        
        # Преобразуем историю сделок в DataFrame
        if self.trade_history:
            self.trade_df = pd.DataFrame(self.trade_history)
                    else:
            self.trade_df = pd.DataFrame(columns=['entry_date', 'exit_date', 'position', 'entry_price', 'exit_price', 'position_size', 'pnl', 'trade_duration', 'market_regime'])
        
        print(f"Backtest completed. Final balance: ${balance:.2f}")
        return balance
    ...
    def plot_regime_performance(self):
        if not hasattr(self, 'trade_df') or self.trade_df is None or len(self.trade_df) == 0 or 'market_regime' not in self.trade_df.columns:
            print("No regime data available or no trades executed. Make sure the backtest has completed successfully.")
            return
    ...
    def analyze_results(self):
        ...
        if self.trade_df is None or len(self.trade_df) == 0:
            print("No trades were executed during the backtest.")
            final_balance = self.backtest_results['balance'].iloc[-1] if not self.backtest_results.empty else self.initial_balance
            return {
                'initial_balance': self.initial_balance,
        ...
        # Используем непосредственно pnl из trade_df, так как он теперь рассчитан корректно
    ...
    def optimize_parameters(self, param_ranges=None, num_trials=80):
...
def main():
    ...
    strategy = BalancedAdaptiveStrategy(
        ...
        max_leverage=2,  # Снижено с 5x до 2x
        base_risk_per_trade=0.01,  # Снижено с 0.07 до 0.01
        min_trades_interval=6  # Увеличено с 1 до 6
    )
